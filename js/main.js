...
function addHeroProof(copy){if(copy.querySelector('.hero-proof'))return;const proof=document.createElement('div');proof.className='hero-proof';proof.innerHTML='<span>Strategy-led</span><span>Human-reviewed</span><span>Built for growth</span>';copy.appendChild(proof)}
function enhancePageHero(){const hero=document.querySelector('.page-hero');if(!hero)return;const key=location.pathname.split('/').pop()||'index.html';const details=HERO_IMAGES[key]||HERO_IMAGES['services.html'];const container=hero.querySelector('.container');if(!container||container.querySelector('.page-hero-visual'))return;const copy=document.createElement('div');copy.className='page-hero-copy';while(container.firstChild)copy.appendChild(container.firstChild);addHeroProof(copy);const visual=document.createElement('div');visual.className='page-hero-visual';const frame=document.createElement('div');frame.className='hero-media-frame';const img=document.createElement('img');img.src=details[0];img.alt=details[1];img.width=1100;img.height=720;img.loading='eager';img.decoding='async';frame.appendChild(img);const topCard=document.createElement('div');topCard.className='hero-float hero-float-top';topCard.innerHTML='<span class="float-icon">↗</span><div><small>International reach</small><strong>Ideas without borders</strong></div>';const bottomCard=document.createElement('div');bottomCard.className='hero-float hero-float-bottom';bottomCard.innerHTML='<span class="pulse-dot"></span><div><small>Our approach</small><strong>Clarity. Craft. Growth.</strong></div>';visual.append(frame,topCard,bottomCard);container.append(copy,visual)}
function enhanceHomeHero(){const hero=document.querySelector('.hero');const copy=hero&&hero.querySelector('.hero-grid>div:first-child');if(copy)addHeroProof(copy);const card=document.querySelector('.hero-card');if(!card||card.querySelector('.hero-image'))return;const media=document.createElement('div');media.className='hero-image-wrap';const img=document.createElement('img');img.className='hero-image';img.src='https://images.unsplash.com/photo-1553877522-43269d4ea984?auto=format&fit=crop&w=1000&q=86';img.alt='Digital marketing professionals planning business growth';img.width=1000;img.height=620;img.loading='eager';img.decoding='async';const label=document.createElement('span');label.className='image-label';label.textContent='Digital growth, made clear';media.append(img,label);card.prepend(media);card.classList.add('has-image')}
enhancePageHero();enhanceHomeHero();

/* Give every blog topic a relevant visual while keeping markup lightweight. */
const BLOG_CARD_IMAGES=[
  ['https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=900&q=82','SEO performance analytics displayed on a screen'],
  ['https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?auto=format&fit=crop&w=900&q=82','Map used to plan local search visibility'],
  ['https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?auto=format&fit=crop&w=900&q=82','Modern website being designed on a computer'],
  ['https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=900&q=82','Marketing budget and paid campaign planning'],
  ['https://images.unsplash.com/photo-1611162617474-5b21e879e113?auto=format&fit=crop&w=900&q=82','Social media applications on a smartphone'],
  ['https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&w=900&q=82','Artificial intelligence visualization for digital marketing']
];
function enhanceBlogCards(){const cards=[...document.querySelectorAll('.blog-grid .card:not(.featured-post)')];cards.forEach((card,index)=>{const details=BLOG_CARD_IMAGES[index];if(!details||card.querySelector('.blog-card-image'))return;const media=document.createElement('div');media.className='blog-card-image';const img=document.createElement('img');img.src=details[0];img.alt=details[1];img.width=900;img.height=560;img.loading='lazy';img.decoding='async';media.appendChild(img);card.prepend(media);card.classList.add('featured-post')})}
enhanceBlogCards();
